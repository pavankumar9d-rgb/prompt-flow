...
- Name: system.test.ts >> System & Specialty Testing >> Visual: Homepage UI snapshot
- Location: src\test\e2e\system.test.ts:28:7

# Error details

```
Error: A snapshot doesn't exist at C:\Users\saipa\OneDrive\Desktop\prompt lib\prompt-flow\src\test\e2e\system.test.ts-snapshots\homepage-obsidian-chromium-win32.png, writing actual.
```
...
```ts
  ...
  3  | 
  4  | test.describe("System & Specialty Testing", () => {
  5  |   test("Smoke Test: Homepage loads correctly", async ({ page }) => {
  6  |     await page.goto("/");
  7  |     await expect(page.locator("h1")).toContainText("Deterministic Engineering");
  8  |   });
  9  | 
  10 |   test("Accessibility: Homepage WCAG compliance", async ({ page }) => {
  11 |     await page.goto("/");
  12 |     await injectAxe(page);
  13 |     await checkA11y(page, undefined, {
  14 |       detailedReport: true,
  15 |       detailedReportOptions: { html: true },
  16 |     });
  17 |   });
  18 | 
  19 |   test("SEO: Meta tags verification", async ({ page }) => {
  20 |     await page.goto("/");
  21 |     const title = await page.title();
  22 |     expect(title).toContain("Prompt-Flow");
  23 |     
  24 |     const description = await page.getAttribute('meta[name="description"]', "content");
  25 |     expect(description).toBeTruthy();
  26 |   });
  27 | 
  28 |   test("Visual: Homepage UI snapshot", async ({ page }) => {
  29 |     await page.goto("/");
  30 |     // Wait for animations to settle
  31 |     await page.waitForTimeout(1000);
> 32 |     await expect(page).toHaveScreenshot("homepage-obsidian.png", {
     |     ^ Error: A snapshot doesn't exist at C:\Users\saipa\OneDrive\Desktop\prompt lib\prompt-flow\src\test\e2e\system.test.ts-snapshots\homepage-obsidian-chromium-win32.png, writing actual.
